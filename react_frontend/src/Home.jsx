...
function SectionTitle({ icon, eyebrow, title, description, compact = false }) {
  return (
    <div className={compact ? "mb-4" : "mb-7"}>
      <div className="mb-2.5 flex items-center gap-2 text-xs tracking-[0.22em] uppercase text-fuchsia-200/80">
        {icon}
        <span>{eyebrow}</span>
      </div>
      <h1 className={`${compact ? "text-2xl md:text-3xl" : "text-3xl md:text-[2.25rem]"} font-semibold text-white`}>
        {title}
      </h1>
      {description && (
        <p className={`${compact ? "mt-2 leading-6" : "mt-3 leading-7"} max-w-2xl text-sm text-slate-300`}>
          {description}
        </p>
      )}
    </div>
...
function shouldSplitExperience() {
  if (typeof window === "undefined") return false;

  const viewportWidth = window.innerWidth;
  const viewportHeight = window.innerHeight;
  const deckHeight = 382;
  const stackedDecksHeight = deckHeight * 2 + 20;
  const sectionTextHeight = 150;
  const sectionPadding = viewportWidth < 768 ? 112 : 160;

  return viewportWidth < 1024 && viewportHeight < stackedDecksHeight + sectionTextHeight + sectionPadding;
}

function useSplitExperienceSections() {
  const [splitExperience, setSplitExperience] = useState(shouldSplitExperience);

  useEffect(() => {
    const updateSplitExperience = () => setSplitExperience(shouldSplitExperience());

    updateSplitExperience();
    window.addEventListener("resize", updateSplitExperience);
    window.addEventListener("orientationchange", updateSplitExperience);

    return () => {
      window.removeEventListener("resize", updateSplitExperience);
      window.removeEventListener("orientationchange", updateSplitExperience);
    };
  }, []);

  return splitExperience;
}
...
function CardDeck({ cards, theme, title, icon }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[280px] flex-col">
      ...
      <div
        className="group relative flex min-h-[356px] w-full items-center justify-center outline-none"
        tabIndex={0}
        onKeyDown={handleKeyDown}
        aria-label={`${title} deck`}
      >
        <div className={`absolute h-[292px] w-[235px] -translate-x-4 translate-y-3 rounded-[1.5rem] border ${styles.ring} bg-white/5 shadow-2xl backdrop-blur-sm`} />
        <div className={`absolute h-[292px] w-[235px] translate-x-4 -translate-y-3 rounded-[1.5rem] border ${styles.ring} bg-white/5 shadow-2xl backdrop-blur-sm`} />
        ...
        <AnimatePresence mode="wait">
          <Motion.div
            ...
            className={`relative z-10 h-[340px] w-[276px] rounded-[1.5rem] border ${styles.ring} bg-gradient-to-b ${styles.glow} p-5 ${styles.shadow} ${isTarot ? "backdrop-blur-sm" : ""} transition-transform duration-300 group-hover:scale-[1.01]`}
          >
            {isTarot ? (
              <div className="relative flex h-full flex-col overflow-hidden rounded-[1.2rem] border border-[#8d7444]/40 bg-[#1a1512]/85 text-[#f1e6cf]">
                ...
                <div className="relative flex h-full flex-col p-3.5">
                  ...
                  <div className="mt-3 flex flex-1 flex-col rounded-[1rem] border border-[#b99a62]/20 bg-[#241d18]/70 px-3.5 py-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.04)]">
                    ...
                    <div className="flex flex-1 flex-col items-center justify-center text-center">
                      <div className="mb-2.5 flex h-14 w-14 items-center justify-center rounded-full border border-[#b99a62]/35 bg-[#faf2df]/8 text-[#d8c08e] shadow-[0_0_18px_rgba(185,154,98,0.12)]">
                        <Moon className="h-7 w-7" />
                      </div>
                      <h3 className="font-serif text-xl uppercase tracking-[0.18em] text-[#f1e6cf]">
                        {current.title}
                      </h3>
                      <div className="mt-3 h-px w-24 bg-[#b99a62]/30" />
                    </div>

                    <p className="mx-auto mt-auto max-w-[220px] text-center text-xs leading-6 text-[#d8c9ab]">
                      {current.text}
                    </p>

                    <div className="mt-3 flex items-center gap-3 text-[#b99a62]/70">
                      <div className="h-px flex-1 bg-[#b99a62]/25" />
                      <Sparkles className="h-4 w-4" />
                      <div className="h-px flex-1 bg-[#b99a62]/25" />
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between text-[10px] uppercase tracking-[0.35em] text-[#b99a62]/80">
                    <span>Arcana</span>
        ...
        <button
          ...
          className="absolute left-1/2 top-1/2 z-20 h-[340px] w-[138px] -translate-x-full -translate-y-1/2 cursor-pointer border-0 bg-transparent p-0 outline-none focus-visible:ring-2 focus-visible:ring-white/30"
        />
        <button
          type="button"
          onClick={goNext}
          aria-label={`Next ${title}`}
          className="absolute left-1/2 top-1/2 z-20 h-[340px] w-[138px] -translate-y-1/2 cursor-pointer border-0 bg-transparent p-0 outline-none focus-visible:ring-2 focus-visible:ring-white/30"
        />
      </div>
    </div>
  );
...
function ExperienceSection({ children, compact = false, title = "Your Original Tarot", description }) {
  return (
    <section className="section">
      <div className={`mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 ${compact ? "py-14" : "py-20"} md:px-10 md:py-20`}>
...
export default function Home() {
  const splitExperience = useSplitExperienceSections();
  const anchors = splitExperience
    ? ["home", "concept", "experience1", "experience2", "author"]
    : ["home", "concept", "experience", "author"];

  useEffect(() => {
    const hash = window.location.hash;

    if (splitExperience && hash === "#experience") {
      window.location.replace("#experience1");
    }
    if (hash === "#experienc1") {
      window.location.replace("#experience1");
    }
    if (hash === "#experienc2") {
      window.location.replace("#experience2");
    }
    if (!splitExperience && (hash === "#experience1" || hash === "#experience2")) {
            ...
            return (
              <ReactFullpage.Wrapper>
                <section className="section relative isolate overflow-hidden">
                  ...
                  <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 pb-20 pt-28 md:px-10 md:pt-32">
                    <div className="home-content-scale max-w-[42rem]">
                      <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-fuchsia-300/20 bg-white/5 px-3.5 py-1.5 text-xs text-fuchsia-100 backdrop-blur-md">
                        <Star className="h-4 w-4" />
                        <span>WELCOME</span>
                    </div>

                    <h1 className="text-4xl font-semibold leading-tight text-white md:text-6xl md:leading-[1.08]">
                      Daily Witchcrafts
                    </h1>
                ...
                <section className="section">
                  <div className="mx-auto flex min-h-screen max-w-[78rem] flex-col justify-center px-6 py-20 md:px-10">
                  <div className="home-content-scale">
                    <SectionTitle
                ...
                {splitExperience ? (
                  <>
                    ...
                    <ExperienceSection
                      compact
                      title="Major Arcana"
                      description="Click the left or right side of the card to move through the deck."
                    >
                      <CardDeck cards={majorArcanaCards} theme="tarot" title="Major Arcana" icon={Sparkles} />
                    </ExperienceSection>
                  </>
                ...
                <section className="section">
                  <div className="mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 py-20 md:px-10">
                    <div className="home-content-scale">
                      <SectionTitle
                      ...
                      <div className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-xl backdrop-blur-md md:p-7">
                        <div className="max-w-4xl">
                          <h3 className="text-xl font-semibold text-white md:text-2xl">Disclaimer(免責事項)</h3>
                          <p className="mt-4 text-sm leading-7 text-slate-300">
